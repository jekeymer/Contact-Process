...
struct simulation
	...
	int init_option;                           // What initial condion map to  use  
	...
	switch(s.init_option)
	  {
		case 0:
	        	r=2;
		        // Add some r2-size cluster with particles                 
		...
		case 2: 
			// Initialize with a  10% lattice occupancy at random
			       for (x = 0; x < X_SIZE; x++)
                for (y = 0; y < Y_SIZE; y++) 
				if(genrand64_real2()<0.1){s.lattice_configuration[x][y]=1;s.occupancy++;}
			break;
...
static void show_about(GtkWidget *widget, gpointer data) 
	...
	gtk_about_dialog_set_program_name (GTK_ABOUT_DIALOG(dialog),
                                    "Contact Process App");	
	gtk_about_dialog_set_version(GTK_ABOUT_DIALOG(dialog), "v 0.2, 2017"); 
...
static void radio_button_toggled(GtkWidget *button, gpointer   user_data)
	{
  	char *b_state;
	...
	g_print ("Initial condition map:  %s,  was turned %s\n", button_label, b_state);
	
	// Perform  the actual change of setting
	if(strcmp(gtk_button_get_label (GTK_BUTTON (button)),"cluster") == 0 && gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button))) 
		s.init_option=0;				
        if(strcmp(gtk_button_get_label(GTK_BUTTON (button)),"line") == 0 && gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button))) 
                s.init_option=1;                                
        if(strcmp(gtk_button_get_label (GTK_BUTTON (button)),"10%") == 0 && gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button))) 
                s.init_option=2;                                
	}
...
static void activate (GtkApplication *app, gpointer user_data)
	...
	s.init_option = 0; // set default initial condition map
	...
	// option 1
	radio_button = gtk_radio_button_new_with_label (NULL, "cluster");
	gtk_grid_attach (GTK_GRID (grid), radio_button, 1, 3, 1, 1);
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (radio_button), TRUE);
	g_signal_connect (GTK_TOGGLE_BUTTON (radio_button), "toggled",G_CALLBACK (radio_button_toggled), window);
	
	// option 2
	radio_button = gtk_radio_button_new_with_label_from_widget (GTK_RADIO_BUTTON (radio_button),"line");
	gtk_grid_attach (GTK_GRID (grid), radio_button, 2, 3, 1, 1);
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (radio_button), FALSE);
	g_signal_connect (GTK_TOGGLE_BUTTON (radio_button), "toggled",G_CALLBACK (radio_button_toggled), window);

	// option 3
        radio_button = gtk_radio_button_new_with_label_from_widget (GTK_RADIO_BUTTON (radio_button), "10%");
        gtk_grid_attach (GTK_GRID (grid), radio_button, 3, 3, 1, 1);
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (radio_button), FALSE);
	g_signal_connect (GTK_TOGGLE_BUTTON (radio_button), "toggled",G_CALLBACK (radio_button_toggled), window);

	// Now the simulation controls	
	// -----    INIT BUTTON   -----
	...
	gtk_grid_attach (GTK_GRID (grid), button, 0, 4, 1, 1); // position (0,4) spanning 1 col and 1 raw)
	// -----  STOP BUTTON    ------
	button = gtk_button_new_with_label ("Stop");
	g_signal_connect (button, "clicked", G_CALLBACK(stop_simulation), NULL);
	gtk_grid_attach (GTK_GRID (grid), button, 1, 4, 1, 1); // position (1,4) spanning 1 col and 1 raw)
	// -----   ABOUT ? BUTTON  ----
	button = gtk_button_new_with_label ("?");
	g_signal_connect (button, "clicked", G_CALLBACK(show_about), GTK_WINDOW(window));
	gtk_grid_attach (GTK_GRID (grid), button, 2, 4, 1, 1); // position (2,4) spanning 1 col and 1 raw)
	//-------   QUIT BUTTON    ----
	button = gtk_button_new_with_label ("Quit"); 
	g_signal_connect_swapped (button, "clicked", G_CALLBACK (gtk_widget_destroy), window);
	gtk_grid_attach (GTK_GRID (grid), button, 3, 4, 1, 1); // position (3,4) spanning 1 col and 1 raw)
